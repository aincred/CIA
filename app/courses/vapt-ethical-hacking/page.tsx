...
// UPDATE: VAPT / Ethical Hacking Syllabus
const courseModules: ModuleData[] = [
    { title: "Lab Environment Setup & Pre-requisites" },
    { 
        title: "Module 1: Intelligence Gathering (Recon)",
        items: [
            "1.1. Passive Footprinting (OSINT)",
            "1.2. Active Reconnaissance",
            "1.3. DNS Enumeration & Mapping",
            "1.4. Google Dorking for Pentesters",
            "1.5. Social Engineering Vectors",
            "1.6. Module Quiz"
        ]
    },
    { 
        ...
        items: [
            "2.1. Mastering Nmap & Zenmap",
            "2.2. Port Scanning Techniques",
            "2.3. Service Version Detection",
            "2.4. OS Fingerprinting",
            "2.5. Vulnerability Scanning with Nessus",
            "2.6. Lab: Mapping the Attack Surface"
        ]
    },
    { 
        title: "Module 3: Vulnerability Assessment",
        items: [
            "3.1. CVSS Scoring System",
            "3.2. Automated vs. Manual Assessment",
            "3.3. Identifying False Positives",
            "3.4. CVE Research & Exploit DB",
            "3.5. Mid-Course Challenge: The Assessment"
        ]
    },
    { 
        title: "Module 4: System Hacking & Exploitation",
        items: [
            "4.1. Metasploit Framework Fundamentals",
            "4.2. Password Cracking (Hashcat/John)",
            "4.3. Privilege Escalation (Linux/Windows)",
            "4.4. Buffer Overflow Basics",
            "4.5. Maintaining Access & Backdoors",
            "4.6. Clearing Tracks",
            "4.7. Lab: Pwning the Box"
        ]
    },
    { 
        title: "Module 5: Web Application Penetration Testing",
        items: [
            "5.1. OWASP Top 10 Deep Dive",
            "5.2. SQL Injection (Manual & SQLmap)",
            "5.3. Cross-Site Scripting (XSS)",
            "5.4. Broken Authentication",
            "5.5. Burp Suite Professional Workflow",
            "5.6. Lab: Hacking a Banking App"
        ]
    },
    { 
        title: "Module 6: Reporting & Professional Ethics",
        items: [
            "6.1. Writing Executive Summaries",
            "6.2. Technical Finding Documentation",
            "6.3. Remediation Consultation",
            "6.4. Legal Frameworks (NDA, MSA)",
            "6.5. Final Practical Exam (CTF Style)"
        ]
    }
];
                    ...
                    <div className="p-2 bg-black/20 border-t border-white/5">
                        {module.items?.map((item, idx) => (
                            <div key={idx} className="flex items-center gap-3 py-2 px-4 hover:bg-white/5 rounded text-sm text-slate-400 transition-colors">
                                <div className={`w-1.5 h-1.5 rounded-full ${item.toLowerCase().includes('quiz') || item.toLowerCase().includes('exam') || item.toLowerCase().includes('lab') ? 'bg-yellow-500' : 'bg-cyan-900'}`} />
                                <span className={item.toLowerCase().includes('quiz') || item.toLowerCase().includes('exam') || item.toLowerCase().includes('lab') ? 'text-white font-semibold' : ''}>
                                    {item}
                                </span>
                            </div>
                        ))}
...
/**
 * MAIN COURSE PAGE - VAPT / ETHICAL HACKING
 */
export default function VAPTCoursePage() {
  ...
  const languages = [
    "English (US)", "English (UK)", "Hindi (हिन्दी)", "Spanish", "French"
  ];
         ...
         <div className="container mx-auto relative z-10">
            <div className="max-w-4xl">
                ...
                <div className="inline-flex items-center gap-2 mb-6">
                    <span className="px-3 py-1 rounded bg-red-950/40 border border-red-500/30 text-red-400 text-xs font-mono font-bold uppercase tracking-wider">
                        Offensive Security
                    </span>
                    <ChevronRight size={12} className="text-slate-600"/>
                    <span className="text-slate-400 text-xs font-bold uppercase tracking-wider">Penetration Tester / Ethical Hacker</span>
                </div>

                <h1 className="text-4xl md:text-6xl font-bold text-white mb-6 leading-tight">
                    VAPT & <span className="text-transparent bg-clip-text bg-linear-to-r from-red-500 to-orange-500">Ethical Hacking</span>
                </h1>
                
                <p className="text-lg text-slate-300 mb-8 max-w-2xl leading-relaxed">
                    Go beyond theory. Learn to think like a hacker to defend like a pro. Master the art of Vulnerability Assessment and Penetration Testing (VAPT) through immersive, real-world attack simulations.
                </p>

                <div className="flex flex-wrap gap-4 mb-10">
                    <StatPill icon={Users} label="Enrolled" value="12,450+" />
                    <StatPill icon={Clock} label="Duration" value="60 Hours" />
                    <StatPill icon={BarChart} label="Level" value="Advanced" />
                    <StatPill icon={Terminal} label="Live Targets" value="25+ Labs" />
                </div>

                <div className="flex flex-col sm:flex-row gap-4">
                    <button className="px-8 py-4 bg-linear-to-r  from-red-600 to-red-700 text-white font-bold rounded-lg hover:from-red-500 hover:to-red-600 transition-all shadow-[0_0_20px_rgba(220,38,38,0.3)] hover:-translate-y-1">
                        Enroll Now
                    </button>
              ...
              <div className="lg:col-span-8 space-y-16">
                  ...
                  <div>
                      ...
                      <div className="prose prose-invert prose-slate max-w-none text-slate-400 leading-relaxed">
                          <p className="mb-4">
                             In the modern digital battlefield, the only way to truly secure an organization is to test its defenses from the perspective of an attacker. This course is designed to take you from a basic understanding of networks to a professional level of penetration testing.
                          </p>
                          <p className="mb-4">
                             You will learn how to systematically scan networks, identify vulnerabilities, craft exploits, and pivot through networks using industry-standard tools like <strong>Kali Linux, Metasploit, Nmap, Burp Suite, and Wireshark.</strong>
                          </p>
                          <p>
                             Crucially, this course emphasizes the "Professional" in VAPT—teaching you how to document findings, calculate risk scores (CVSS), and communicate remediation strategies to stakeholders.
                          </p>
                      </div>
                  </div>
                  ...
                  {/* Skills */}
                  <div>
                      <h3 className="text-xl font-bold text-white mb-6">Skills You Will Master</h3>
                      <div className="flex flex-wrap gap-3">
                          {["Penetration Testing", "OWASP Top 10", "Network Scanning", "Exploit Development", "Privilege Escalation", "Burp Suite", "Metasploit", "Vulnerability Assessment", "Python for Pentesting"].map((skill) => (
                              <span key={skill} className="px-4 py-2 rounded-full bg-slate-900 border border-red-900/40 text-red-300 text-sm font-medium shadow-sm">
                                  {skill}
                              </span>
                          ))}
                      </div>
                  ...
                  {/* Curriculum */}
                  <div>
                        <h3 className="text-2xl font-bold text-white mb-6">Detailed Syllabus</h3>
                  ...
                  <div className="bg-linear-to-r  from-slate-900 to-slate-900/50 border border-cyan-900/30 rounded-2xl p-8 flex items-center justify-between">
                        <div>
                           <h3 className="text-xl font-bold text-white mb-2">Certified Ethical Hacker Badge</h3>
                           <p className="text-slate-400 text-sm">Pass the final CTF exam to earn your cryptographically verified digital credential.</p>
                        </div>
                        <div className="w-16 h-16 bg-red-900/20 rounded-full flex items-center justify-center border-2 border-red-500/50 shadow-[0_0_15px_rgba(220,38,38,0.3)]">
              ...
              <div className="lg:col-span-4">
                  <div className="sticky top-28 space-y-6">
                      ...
                      <div className="bg-slate-900/80 backdrop-blur border border-white/10 rounded-2xl p-6 shadow-xl">
                          <h4 className="text-white font-bold mb-4">At a Glance</h4>
                          <div className="space-y-4">
                              <div className="flex justify-between items-center py-2 border-b border-white/5">
                                  <span className="text-slate-400 text-sm">Price</span>
                                  <div className="text-right">
                                    <span className="text-slate-500 line-through text-xs block">₹24,999</span>
                                    <span className="text-white font-bold text-lg">₹14,999</span>
                                  </div>
                              </div>
                              <div className="flex justify-between items-center py-2 border-b border-white/5">
                                  <span className="text-slate-400 text-sm">Mode</span>
                                  <span className="text-white font-bold text-right">Live Instructor-Led</span>
                              </div>
                              <div className="flex justify-between items-center py-2 border-b border-white/5">
                                  <span className="text-slate-400 text-sm">Prerequisites</span>
                                  <span className="text-white font-bold text-right text-xs">Linux & Networking Basics</span>
                              </div>
                              <div className="flex justify-between items-center py-2 border-b border-white/5">
                                  <span className="text-slate-400 text-sm">Certification</span>
                                  <span className="text-white font-bold">CIA-VAPT</span>
                              </div>
                          </div>
                          <button className="w-full mt-6 py-3 bg-red-600 text-white font-bold rounded hover:bg-red-500 transition-colors shadow-lg shadow-red-900/20">
                              Secure Your Spot
                          </button>
                      </div>
                      ...
                      <div className="bg-slate-900/40 border border-white/5 rounded-2xl p-6">
                          <h4 className="text-white font-bold mb-4 flex items-center gap-2">
                              <Globe size={16} className="text-slate-500"/> Available Languages
                          </h4>
                          <div className="flex flex-wrap gap-2">
                              {languages.map((lang, i) => (
                                  <span key={i} className="text-xs text-slate-500 hover:text-cyan-400 cursor-pointer transition-colors border-b border-transparent hover:border-cyan-500/50">
                                      {lang}
                                  </span>
                              ))}
                          </div>
