...
// import React, { useState, ChangeEvent, FormEvent } from 'react';
// import { Shield, User, GraduationCap, FileText, CheckCircle, AlertTriangle, Terminal, Upload, Lock, Calendar, Download, Phone, Mail, CheckSquare } from 'lucide-react';
...
//   // File Uploads
//   photo: File | null;
//   fileAadhaar: File | null;
//   fileCollegeId: File | null;
//   fileNoc: File | null; // Optional

//   // Checklist Booleans (Matching the screenshot)
//   checkAadhaar: boolean;
//   checkCollegeId: boolean;
//   checkNoc: boolean;
//   checkPhoto: boolean;
...
//   // Specific handler for 10-digit mobile numbers
...
//     if (formData.studentContact.length !== 10 || formData.parentContact.length !== 10) {
...
//       if (formData.photo) photoBase64 = await convertToBase64(formData.photo);
//       if (formData.signatureFile) signatureBase64 = await convertToBase64(formData.signatureFile);
//       if (formData.fileAadhaar) aadhaarBase64 = await convertToBase64(formData.fileAadhaar);
//       if (formData.fileCollegeId) collegeIdBase64 = await convertToBase64(formData.fileCollegeId);
//       if (formData.fileNoc) nocBase64 = await convertToBase64(formData.fileNoc);
...
//         collegeIdBase64,
//         nocBase64,
//         // Remove raw File objects before sending
//         photo: undefined, 
//         signatureFile: undefined,
//         fileAadhaar: undefined,
//         fileCollegeId: undefined,
//         fileNoc: undefined
//       };
...
//       if (response.ok && data.success) {
//         setGeneratedBatchCode(data.batchCode || 'CIA/B1/001');
//         setGeneratedRegNo(data.registrationNo || 'CIA/B1');
//         setSubmitted(true);
//         window.scrollTo(0, 0);
//       } else {
//         alert(`Registration failed: ${data.message || 'Unknown error'}`);
...
//   // Helper for document mapping to match your image exactly
//   const checklistItems = [
//     { key: 'checkAadhaar', label: 'Aadhaar Card Copy' },
//     { key: 'checkCollegeId', label: 'College ID Card Copy' },
//     { key: 'checkNoc', label: 'NOC from College (If required)' },
//     { key: 'checkPhoto', label: 'Passport Photo' }
//   ];
...
//               <div className="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6">
//                 <Input 
//                   label="Student Contact No. (10 Digits)" 
//                   name="studentContact" 
//                   value={formData.studentContact} 
...
//                 />
//                 <Input 
//                   label="Parent/Guardian Contact No. (10 Digits)" 
//                   name="parentContact" 
//                   value={formData.parentContact} 
//                   onChange={handlePhoneChange} 
//                   type="tel"
...
//           {/* Section 3: Document Uploads & Checklist */}
//           <Section title="3. Upload Documents" icon={<FileText className="w-5 h-5" />}>
...
//                   name="fileAadhaar" 
//                   file={formData.fileAadhaar} 
//                   onChange={handleFileChange} 
//                   required={true}
//                 />
//                 <FileUploadField 
//                   label="Upload College ID Card Copy" 
//                   name="fileCollegeId" 
//                   file={formData.fileCollegeId} 
//                   onChange={handleFileChange} 
//                   required={true}
//                 />
//                 <FileUploadField 
//                   label="Upload NOC from College (If Required)" 
//                   name="fileNoc" 
//                   file={formData.fileNoc} 
//                   onChange={handleFileChange} 
//                   required={false}
//                 />
//               </div>

//               {/* Part B: Documents Submitted Checklist (New Option Added After Upload) */}
...
//                             onChange={handleChange}
//                             className="peer h-5 w-5 cursor-pointer appearance-none rounded border border-slate-600 checked:bg-emerald-500 checked:border-emerald-500 transition-all bg-slate-950"
//                           />
//                           <CheckCircle className="absolute hidden peer-checked:block text-slate-900 pointer-events-none" size={14} style={{left: '3px', top: '3px'}} />
//                         </div>
//                         <span className={`text-sm ${
//                           // @ts-ignore
//                           formData[item.key] ? 'text-white' : 'text-slate-400'
//                         }`}>
//                           {item.label}
//                         </span>
//                       </label>
...
// function FileUploadField({ label, name, file, onChange, required = false }: FileUploadFieldProps) {
//   return (
//     <div className="space-y-1">
//       <label className="block text-sm font-medium text-slate-400">
//         {label} {required && <span className="text-emerald-500">*</span>}
//       </label>
//       <div className="relative w-full bg-slate-950 border border-slate-700 border-dashed rounded-lg p-3 flex items-center justify-between hover:border-emerald-500 hover:bg-slate-900 transition-all cursor-pointer group">
...
//                <span className="text-sm font-medium text-emerald-400 truncate max-w-50 md:max-w-xs">{file.name}</span>
...
interface RegistrationFormData {
  ...
  dob: string;
  gender: string;
  aadhaarLast4: string; // Kept the number field, removed the file
  studentContact: string;
  parentContact: string;
  email: string;
  address: string;
  ...
  // File Uploads
  photo: File | null;
  // fileAadhaar: File | null; // REMOVED
  fileCollegeId: File | null;
  fileNoc: File | null; // Optional

  // Checklist Booleans
  // checkAadhaar: boolean; // REMOVED
  checkCollegeId: boolean;
  checkNoc: boolean;
  checkPhoto: boolean;
...
export default function RegistrationForm() {
  ...
  const initialFormState: RegistrationFormData = {
    ...
    // Document Files
    // fileAadhaar: null, // REMOVED
    fileCollegeId: null,
    fileNoc: null,

    // Checklist Booleans
    // checkAadhaar: false, // REMOVED
    checkCollegeId: false,
    checkNoc: false,
    checkPhoto: false,
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      let photoBase64 = '';
      let signatureBase64 = '';
      // let aadhaarBase64 = ''; // REMOVED
      let collegeIdBase64 = '';
      let nocBase64 = '';

      if (formData.photo) photoBase64 = await convertToBase64(formData.photo);
      if (formData.signatureFile) signatureBase64 = await convertToBase64(formData.signatureFile);
      // if (formData.fileAadhaar) aadhaarBase64 = await convertToBase64(formData.fileAadhaar); // REMOVED
      if (formData.fileCollegeId) collegeIdBase64 = await convertToBase64(formData.fileCollegeId);
      if (formData.fileNoc) nocBase64 = await convertToBase64(formData.fileNoc);
      ...
      const payload = {
        ...
        photoBase64,
        signatureBase64,
        // aadhaarBase64, // REMOVED
        collegeIdBase64,
        nocBase64,
        // Remove raw File objects before sending
        photo: undefined, 
        signatureFile: undefined,
        // fileAadhaar: undefined, // REMOVED
        fileCollegeId: undefined,
        fileNoc: undefined
      };
  ...
  // Adjusted checklist Items (Removed Aadhaar)
  const checklistItems = [
    // { key: 'checkAadhaar', label: 'Aadhaar Card Copy' }, // REMOVED
    { key: 'checkCollegeId', label: 'College ID Card Copy' },
    { key: 'checkNoc', label: 'NOC from College (If required)' },
    { key: 'checkPhoto', label: 'Passport Photo' }
  ];
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <Section title="3. Upload Documents" icon={<FileText className="w-5 h-5" />}>
            <div className="space-y-6">
              ...
              {/* Part A: File Inputs */}
              <div className="grid grid-cols-1 gap-4">
                {/* REMOVED AADHAAR UPLOAD COMPONENT */}
              ...
              {/* Part B: Documents Submitted Checklist */}
              <div className="border-t border-slate-700 pt-6">
                <div className="bg-slate-900/50 border border-slate-800 rounded-lg p-5">
                  <h3 className="text-sm font-bold text-slate-300 uppercase tracking-wide mb-4 flex items-center gap-2">
                    <CheckSquare className="w-4 h-4 text-emerald-500" />
